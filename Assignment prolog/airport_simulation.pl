...
isFlightCheap(A,C,B):- flight(A,C,B,P,D), P<400.

/* is it possible to go from A to B in two flights*/
isTwoFlight(A, B):- isFlightExist(A, X), isFlightExist(X,B), A \= B.
...
isFlightUnited(A, B):- flight(A, C, B, P, D), C == united.

/* checking if there is flight from A to B with airline aircanada*/
isFlightAirCanada(A, B):- flight(A, C, B, P, D),C == aircanada.

/* Checking it with and*/
isFlight(A, B):- isFlightUnited(A, B), isFlightAirCanada(A, B).

/* Print particular flight*/
